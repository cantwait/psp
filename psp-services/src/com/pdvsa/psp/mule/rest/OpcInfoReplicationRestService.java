...
@Path("/")
public class OpcInfoReplicationRestService {
...
//			getItemService().saveItem(nuevoItem);
			
		}else{			
			Item actualizarItem = actualizarItem(item, iRef);			
//			getItemService().saveItem(actualizarItem);
		}		
	...
	public GrupoTransferencia transferGroups(GrupoTransferencia gRef){
		...
		Grupo grupo = getGrupoService().getGrupoByNombre(gRef.getGrupo().getNombre());
	...
	public ServidorTransferencia transferServers(ServidorTransferencia sRef){		
		ServidorOpc servidor = getServidorService().getServidorByNombre(sRef.getServidor().getNombre());
		System.out.println("NOMBRE SERVIDOR: " + sRef.getServidor().getNombre() + " CANTIDAD DE TANQUES: " + sRef.getTanques().size() + " CANTIDAD DE GRUPOS X SERVIDOR: " + sRef.getServidorGrupo().size());
		return sRef;
	}
	...
	private Item generarNuevoItem(Item itemRef){
		...
		if(itemRef.getUnidadMedida() != null){
			UnidadMedida um = getUnidadService().getUnidadMedidaByNombre(itemRef.getUnidadMedida().getNombre());
			if(um != null){
